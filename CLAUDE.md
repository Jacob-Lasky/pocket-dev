...
**`dgssh` is the ssh-shaped sibling of `dgvpn`.** ssh ignores `HTTP_PROXY`, so the env-var trick `dgvpn` uses does not reach it. `dgssh` instead points ssh's `ProxyCommand` at `dgvpn-connect`, which speaks HTTP CONNECT to the same proxy and hands ssh a raw byte stream — the proxy resolves `.consul` and dials the tailnet host itself (`proxy.go`'s `handleConnect`). So the in-container Claude can `ssh aws-tools` / `rsync -e dgssh …` to run code on tailnet hosts, e.g. the request-raid live-test recipe.
...
Two runtime prerequisites `dgssh` does NOT bake into the image (deliberately — they are Deepgram creds, kept out of the public image and out of git per the Enterprise-migration "off personal creds" direction): a `~/.ssh/config` Host block whose `HostName` is the **`.consul`** name (a bare IP dials direct and fails — no exit node), and a Vault-OIDC-signed SSH cert (`CertificateFile`). Drop both in at runtime.

**Architecture lives in `vpn/`.** A static Go binary (`dgvpn-proxy`) holds one userspace `tsnet` identity and runs a localhost HTTP CONNECT/forward proxy. `main.go` is the single-identity lifecycle (tsnet up, `RouteAll=true`, surface the SSO URL, start proxy); `proxy.go` is lifted from deephive's `services/tsnet/proxy.go`; `connect.go` is the CONNECT client for the `dgssh` ProxyCommand, a busybox-style second mode of the same binary (dispatched by argv0 `dgvpn-connect` or a `connect` first arg), so the ssh path adds no runtime dependency. It runs as the unprivileged `claude` user: userspace tsnet needs no TUN device and no `NET_ADMIN`. Go tests: `cd vpn && go test ./...`.

**DO NOT replace the Go sidecar with `tailscale up` / `tailscaled`.** This is the load-bearing reason the code exists. Userspace `tailscaled` cannot resolve `.consul` split-DNS for outbound connections (tailscale#16906, tailscale#4677): it answers only globally-resolvable or MagicDNS names, and `.consul` is neither. The sidecar works around this exactly as deephive did (#380), resolving names itself via the tsnet LocalAPI (`proxy.go`'s `resolveViaLocalAPI`) before dialing. Kernel-mode `tailscaled` would also need a TUN device and `NET_ADMIN`, which this unprivileged container does not have. If you "simplify" this to a stock tailscale client, `.consul` silently stops resolving. This is also why `dgssh` reaches tailnet ssh hosts by tunnelling through the CONNECT proxy rather than by joining the box to the tailnet with `tailscaled`: same constraint, same reason.
