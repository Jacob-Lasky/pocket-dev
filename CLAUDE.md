...
`reset()` is followed by `term.scrollToBottom()`, and that second call is load-bearing. `reset()` empties the buffer but does not reliably clear xterm's "the user scrolled up, stop auto-following" state, so the replay lands while the pane is still pinned above the bottom and the visible window straddles the pre-restart paint and the replayed one. Measured in CI after a node-only restart: `baseY 2, viewportY 0`, giving two copies of the same line ON SCREEN while the bottom-anchored screen was correct. Reliable on webkit, intermittent on chromium and firefox, which is exactly why it read as flake.
