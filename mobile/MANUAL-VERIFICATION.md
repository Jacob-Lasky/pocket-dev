...
- [ ] Each Claude or Codex tab that was mid-work resumes its conversation and picks the work back up on its own, having been asked "continue please" exactly once. It arrives as the provider's first message, not as keystrokes. Check each conversation shows one such user turn, not two.
- [ ] The tab that was waiting on you resumes its conversation and just sits there. Nothing is typed into it.
- [ ] Both Codex tabs resume their own distinct conversations. Neither opens fresh and neither attaches to the other tab's conversation. An interrupted or in-progress turn receives the continuation prompt; a completed or failed turn receives none.
- [ ] That decision is read on the boot path, so time it: with several Codex tabs open, restore still completes in seconds, and `docker logs pocket-dev` carries one line per Codex tab naming the turn state and NO thread id anywhere. A boot that hangs, or a log that quotes a conversation id, is a regression in `pd-codex-thread-status` rather than in restore.
...
- [ ] Run `/clear` in a Codex tab, restart, and confirm the cleared conversation resumes. Its official `SessionStart.source: clear` event must atomically replace the prior binding even though the Codex process did not exit. Then run `/compact`, restart, and confirm the same conversation resumes; compact must preserve the established root because measured compact-era lifecycle ids were present in neither the rollout nor state index. Finally check `$PD_STATE_DIR/sids` holds only `<id>.uuid` files — a leftover invocation `*.tmp` means the publish path failed halfway.
...
- [ ] Session list → **+ Codex DG** and **+ Codex GPT** each start Codex's own TUI rather than an error or a bare shell. If either dies immediately, read the pane: an unrecognised flag means a Claude-only argument reached the Codex command line. **Note `--skip-git-repo-check` is NOT on either tab's command line and must not be added**: measured against codex-cli 0.151.0 it exists on `codex exec` only, and bare `codex --skip-git-repo-check` exits with `unexpected argument`. A tab starts in `$HOME`, which is not a repo, so answer Codex's own trust gate in the TUI.
- [ ] Read the model off both Codex banners. Codex DG must name `gpt-5.6-sol`; the registry pins that with `-m` because the Deepgram API account owns that model/billing decision. Codex GPT inherits the ChatGPT account default so connected-app compatibility stays with that account. Before changing the DG pin, confirm a newer Sol EXISTS using the must-fail-control recipe in `~/.claude/skills/second-opinion/SKILL.md` `<model_choice>`: `gpt-6-sol` returns the same 400 as a deliberately bogus id. Do NOT read the banner alone as proof of a bump: it echoes invalid model strings too.
- [ ] In Codex GPT, make read-only calls to one connected Slack tool and one connected Notion tool. Both must succeed. Run the same probe in Codex DG and confirm the app tools are unavailable, proving the picker did not silently reuse the wrong credential route.
- [ ] Send a prompt from the mobile composer in each Codex route. One tap must start the turn; if the prompt only appears in Codex's input box, the Kitty Enter sequence or its paste-settle delay regressed. Submit two requests concurrently to one disposable tab and confirm they become two turns rather than one merged prompt.
- [ ] The rows read **`Codex (Deepgram) · main-N`** and **`Codex (ChatGPT) · main-N`**, both with **`Status not tracked`**, a grey dot, no unread dot, and a relative time that still ticks. A bare `main-N` means the provider label is not reaching the row; `Waiting on you` means the opaque state is not being applied and the tab is lying about needing you.
...
- [ ] `docker restart pocket-dev` with one Claude tab and one Codex tab open. Both come back on their OWN harness and conversation. `docker logs pocket-dev` names the Codex turn as active, completed, failed, or unavailable, and the Codex pane is Codex rather than Claude. A Codex tab that comes back running Claude means the restart loop is baking a process-wide command again.
