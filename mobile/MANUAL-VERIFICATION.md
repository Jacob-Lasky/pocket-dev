...
- [ ] `command -v codex` is `~/bin/codex` once the remote-control block has run (the standalone launcher, which is expected) or `/usr/local/bin/codex` with `PD_CODEX_RC=0`. Either way it must be a plain launcher and not a wrapper: run a consult and confirm its banner says `sandbox: read-only`, which is the only visible proof no bypass crept in. `~/bin/codex-dg` may exist beside it and must NOT be what a bare `codex` resolves to.
...
- [ ] The tab's command line is `codex-dg`, not bare `codex`, so it bills the Deepgram API account: check the pane came up at all (an unprovisioned host has no `~/bin/codex-dg` and the tab dies with `command not found`, which is the intended fail-closed) and that `~/.codex/deepgram-openai.env` and `~/.codex/deepgram.config.toml` are both present. Provisioning is `~/.claude/skills/second-opinion/codex-dg/install.sh` and is deliberately outside this image.
- [ ] Read the model off the Codex tab's banner: it must name `gpt-5.6-sol`. That pin is only valid through `codex-dg` — Sol exists on the Deepgram API account and returns a 400 on the ChatGPT seat, so a tab that 400s every prompt while the banner still says Sol means the wrapper is gone and the pin was left behind. The registry pins it with `-m` on the tab's command line, which is the only thing pinning it (PR #55 removed the `~/.codex/config.toml` seed, and the base config that `codex login` also writes is deliberately left alone). Inheriting whatever the account's default happens to be is NOT the fallback to want: measured 2026-09-08 it was a different model family on both accounts. Before bumping the pin, confirm a newer Sol EXISTS using the must-fail-control recipe in `~/.claude/skills/second-opinion/SKILL.md` `<model_choice>`: `gpt-6-sol` returns the same 400 as a deliberately bogus id. And do NOT read the banner as proof of a bump — it echoes any `-m` string without validating it, so `-m sol` prints a plausible banner for a model that does not exist.
...
- [ ] Confirm `/second-opinion` still works from inside a Claude session, and that its banner still says `sandbox: read-only`. The Codex tab's command line carries the sandbox bypass, and the thing that must remain true is that a consult reaches a plain `codex` (the standalone launcher in `~/bin` is expected and adds no flags; `codex-dg` must not be it), so a consult is unaffected.

## Codex remote control (needs a real ChatGPT login with MFA satisfied and a phone: nothing in CI starts the daemon or reaches chatgpt.com)
- [ ] `docker logs pocket-dev` carries `codex remote-control daemon started`. On a first boot it is preceded by `installed the standalone codex`, and `~/.codex/packages/standalone/current/bin/codex` plus the `~/bin/codex` launcher both exist afterwards.
- [ ] Recreate the container. The standalone is already there, so the install line does not repeat and the daemon still starts — that is the whole reason `CODEX_INSTALL_DIR` is `~/bin` and not `~/.local/bin`, which would not survive.
- [ ] Start a Codex tab and confirm it shows up in the Codex remote-control list on the phone, and that driving it there moves the same pane in the browser.
- [ ] Break it on purpose: `PD_CODEX_RC=0`, restart. No install, no daemon, no error — Codex tabs still start and the terminal is unaffected. Then log codex out and restart with the block on: the failure is two lines in `docker logs` and nothing else.
